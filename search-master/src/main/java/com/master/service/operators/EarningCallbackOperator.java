...
@Slf4j
@Service
public class EarningCallbackOperator extends WorkerCallbackOperator {

    @Value("${EARNING_CALLBACK_HANDLER_KEY:EARNING_CALLBACK}")
    private String HANDLER_KEY;
    @Value("${EARNING_CALLBACK_OPERATOR_ACTIVE:true}")
    private boolean OPERATOR_ACTIVE;

    @Value("${EARNING_CALLBACK_DATA_TYPE:earning}")
    private String DATA_TYPE;

    @Value("${EARNING_CALLBACK_CONFIDENCE:1.0}")
    private double CONFIDENCE;

    @Value("${EARNING_CALLBACK_OPERATOR_TIMEOUT:2000}")
    private int TIMEOUT;

    @Value("${EARNING_CALLBACK_REMOTE_URL:http://earning-search-worker:8080/v1/api/search}")
    private String REMOTE_URL;
    @Value("${EARNING_CALLBACK_RESULT_DATA_KEY:data}")
    private String RESULT_DATA_KEY;
    @Value("${EARNING_CALLBACK_RESULT_DATA_KEY:1}")
    private String SS_CODE;
    @Value("${EARNING_CALLBACK_RESULT_DATA_KEY:code}")
    private String SS_CODE_KEY;

    @Value("${EARNING_CALLBACK_CACHE_TYPE:REDIS}")
    protected String CACHE_TYPE;
    @Value("${EARNING_CALLBACK_CACHE_MAX_SIZE:1000}")
    protected Integer CACHE_MAX_SIZE;
    @Value("${EARNING_CALLBACK_CACHE_CAPACITY:100}")
    protected Integer CACHE_CAPACITY;
    @Value("${EARNING_CALLBACK_CACHE_EXPIRE_TIME:60}")
    protected Integer CACHE_EXPIRE_TIME;

    @Value("${EARNING_CALLBACK_REDIS_HOST:redis}")
    protected String REDIS_HOST;
    @Value("${EARNING_CALLBACK_REDIS_PORT:6379}")
    protected Integer REDIS_PORT;
    @Value("${EARNING_CALLBACK_REDIS_PASSWORD:9a46259f1b75feaa}")
    protected String REDIS_PASSWORD;
    @Value("${EARNING_CALLBACK_REDIS_DATABASE:0}")
    protected Integer REDIS_DATABASE;

    @Value("${EARNING_CALLBACK_REDIS_PROTOCOL_TIMEOUT:2000}")
    protected Integer REDIS_PROTOCOL_TIMEOUT;
    @Value("${EARNING_CALLBACK_REDIS_MAX_IDLE:3}")
    protected Integer REDIS_MAX_IDLE;
    @Value("${EARNING_CALLBACK_REDIS_MAX_TOTAL:1000}")
    protected Integer REDIS_MAX_TOTAL;
    @Value("${EARNING_CALLBACK_REDIS_MAX_WAIT_MILLIS:10}")
    protected Integer REDIS_MAX_WAIT_MILLIS;

    @Value("${EARNING_CALLBACK_REDIS_MIN_EVICTABLE_IDLE_TIME_MILLIS:1000}")
    protected Long REDIS_MIN_EVICTABLE_IDLE_TIME_MILLIS;
    @Value("${EARNING_CALLBACK_REDIS_NUM_TESTS_PER_EVICTION_RUN:3}")
    protected Integer REDIS_NUM_TESTS_PER_EVICTION_RUN;
    @Value("${EARNING_CALLBACK_REDIS_TIME_BETWEEN_EVICTION_RUNS_MILLIS:3}")
    protected Integer REDIS_TIME_BETWEEN_EVICTION_RUNS_MILLIS;
    @Value("${EARNING_CALLBACK_REDIS_TEST_ON_BORROW:true}")
    protected Boolean REDIS_TEST_ON_BORROW;
    @Value("${EARNING_CALLBACK_REDIS_TEST_WHILE_IDLE:true}")
